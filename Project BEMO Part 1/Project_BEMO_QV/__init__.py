...
class Player(BasePlayer):
    ...
    # Defining Variable for using in the Class Instructions
    is_mobile = models.BooleanField()

    # Defining Variable for using in the Class Information
    f_name = models.StringField(label='First Name')
    l_name = models.StringField(label='Last Name')
    stud_id = models.StringField(label='Campus ID')
    email_id = models.StringField(label='UMBC Email Address')
    payment_meth = models.StringField(
        label='Select Payment Method',
        choices=['Paypal', 'Amazon Gift Card', 'Venmo'],
    )
    
    # Defining Variable for using in the Class Decision
    ...
    def set_sure_payoffs(self):
        ...
        csv_file_path = 'output\output_QV.csv'
...
class Information(Page):
    form_model = 'player'
    form_fields = ['f_name', 'l_name', 'stud_id', 'email_id', 'payment_meth']
...
class Results(Page):
    ...
    def is_displayed(player: Player): 
        return player.subsession.round_number == 68
...
page_sequence = [Instructions, Information, Video, Decision, Results]

def custom_export(players):
    # header row
    yield ['session', 'participant_code', 'participant_label', 'First Name', 'Last Name', 'Campus ID', 'UMBC Email Address', 'Payment Method', 'uid' ,'choice', 'Payout_High', 'Payout_Middle', 'Payout_Low', 'Left_High', 'Left_Middle', 'Left_Low', 'Right_High', 'Right_Middle', 'Right_Low', 'Page_Time', 'Time_Started_UTC', 'Survey_ID']
    for p in players:
        participant = p.participant
        session = p.session
        yield [session.code, participant.code, participant.label, p.f_name, p.l_name, p.stud_id, p.email_id, p.payment_meth, p.u_id, p.choice, p.pay_h, p.pay_m, p.pay_l, p.left_h, p.left_m, p.left_l, p.righ_h, p.righ_m, p.righ_l, p.page_time, participant.time_started_utc, p.survey_id]
